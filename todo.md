...
## 관리자 기능 추가 (2026-02-03)
- [x] 관리자 대시보드에 모든 유저 주행 기록 조회 기능 추가 (최근순)
- [x] 웹 관리자 페이지에 모든 유저 주행 기록 조회 기능 추가

## 그룹 주행 버그 수정 (2026-02-03)
- [x] 그룹 주행 시 그룹장만 딜레이 발생하는 문제 수정 (주기적 브로드캐스트 500ms, 콜백 ref 사용)
- [x] 그룹장 앱이 오래 주행하면 렉 걸리는 문제 수정 (멤버 업데이트 쓰로틀링, 로그 제거, 브로드캐스트 간격 조정)

## 프로필 및 그룹 기능 개선 (2026-02-03)
- [x] 그룹원 접속 상태 표시 - 접속 중/미접속 프로필 오버레이에서 명확하게 구분
- [x] 프로필 색상 다양화 - 주황색 외에 다양한 색상 선택 가능
- [x] 프로필 사진 서버 저장 - 서버에 업로드하여 영구 저장 및 다른 사용자에게 표시

## 프로필 기능 추가 개선 (2026-02-03)
- [x] 그룹 멤버 프로필 색상 표시 - 그룹 주행 오버레이에서 각 멤버의 프로필 색상 표시
- [x] 프로필 색상 실시간 미리보기 - 색상 선택 시 실시간으로 프로필 미리보기 표시 (이미 구현됨)

## 기능 개선 및 버그 수정 (2026-02-04)
- [x] 주행기록 고도/속도 스크롤링 및 그래프 토글 기능 추가
- [x] 업적과 뱃지 시스템 동작 구현
- [x] 목표설정 기능 동작 구현 (날짜 파싱 버그 수정)
- [x] 커뮤니티 카테고리별 탭 추가 + 전체 글 보기
- [x] 푸시 알림 시스템 점검 (정상 구현 확인)
- [x] 주행기록 공유/최고속도 기록 날짜 표시 버그 수정 (Invalid Date)
- [x] 키보드가 입력란을 가리는 문제 해결 (KeyboardAvoidingView 적용)

## 주행 기록 날씨 정보 연동 (2026-02-04)
- [ ] 기상청 API 연동 서비스 구현
- [ ] 데이터베이스에 날씨 필드 추가 (온도, 습도, 풍속, 날씨상태)
- [ ] 주행 시작 시 출발지 날씨 정보 자동 수집
- [ ] AI 분석 리포트에 날씨 정보 반영

## 주행 기록 날씨 정보 연동 (2026-02-04)
- [x] 기상청 API 연동 서비스 구현 (server/weather.ts)
- [x] 데이터베이스에 날씨 필드 추가 (온도, 습도, 풍속, 날씨상태)
- [x] 주행 시작 시 출발지 날씨 정보 자동 수집 (riding.tsx)
- [x] AI 분석 리포트에 날씨 정보 반영 (weather_impact 필드 추가)
- [ ] 기상청 API 키 활성화 후 실제 테스트 (내일 확인 예정)

## AI 분석 강화 및 날씨 기능 확장 (2026-02-04)
- [x] AI 분석에 가속도 데이터 추가 (GPS 속도 변화율 계산)
- [x] AI 분석에 경사도/고도 변화 데이터 추가
- [x] AI 분석에 급가속/급감속 횟수 추가
- [x] AI 분석에 정지 횟수 추가
- [x] 주행 기록 상세 화면에 날씨 정보 표시 (아이콘, 온도, 습도, 풍속)
- [x] 날씨별 주행 통계 화면 구현 (맑음/흐림/비/눈 카테고리)
- [x] 날씨 기반 주행 추천 기능 (현재 날씨에 따른 안전 팁)
- [x] 홈 화면에 날씨별 통계 메뉴 추가
- [x] 주행 화면에 날씨 팁 표시

## 날씨 기능 추가 개선 (2026-02-04)
- [x] 기상청 API 테스트 (API 키 활성화 확인) - 성공!
- [x] 날씨별 연비 비교 그래프 구현 (맑음/흐림/비/눈 조건별 연비 차트)
- [x] 주행 경로별 날씨 변화 표시 (장거리 주행 시 구간별 날씨 기록)

## 주행시간 버그 수정 및 날씨 기능 확장 (2026-02-04)
- [x] 주행시간 측정 버그 수정 (총 시간 = 주행시간 + 휴식시간 불일치 문제)
  - RidingRecord에 restTime, totalTime 필드 추가
  - 수동 일시정지 시에도 휴식시간 카운트
  - ride-detail에서 startTime/endTime 기반 정확한 휴식시간 계산
- [x] 날씨 변화 감지 시 푸시 알림 기능
  - notifyWeatherChange: 날씨 변화 알림 (비/눈 시작 등)
  - notifyWeatherWarning: 강풍/한파/폭염 경고
- [x] 월간 날씨별 주행 리포트 화면 구현
  - 월별 날씨 카테고리별 통계
  - 온도 범위 및 인사이트 제공
  - 홈 화면에 메뉴 추가

## 리포트 내보내기, 날씨 위젯 및 안정성 점검 (2026-02-04)
- [ ] 월간 리포트 PDF/이미지 내보내기 기능
- [ ] 홈 화면 날씨 위젯 (현재 날씨 + 주행 추천)
- [ ] 주행 중 강제종료 문제 분석 및 코드 안정성 점검
  - [ ] riding.tsx 메모리 누수 점검
  - [ ] GPS 위치 추적 안정성 점검
  - [ ] 비동기 작업 에러 핸들링 강화
  - [ ] 백그라운드 작업 안정성 점검

## 리포트 내보내기, 날씨 위젯 및 안정성 점검 (2026-02-04)
- [x] 월간 리포트 이미지 내보내기 기능 구현
- [x] 홈 화면 날씨 위젯 구현 (현재 날씨, 주행 추천 여부)
- [x] 주행 중 강제종료 문제 해결을 위한 코드 안정성 점검
  - handleLocationUpdate 전체 try-catch 감싸기
  - 유효하지 않은 GPS 좌표 필터링
  - 날씨 API 호출 에러 처리 강화
  - 그룹 위치 업데이트 에러 처리 강화
  - Haptics 호출 에러 처리 추가
...
## 자동 복구, 성능 모니터링 및 APK 빌드 (2026-02-04)
- [x] 주행 중 자동 복구 기능 - 앱 강제종료 후 재시작 시 이전 주행 데이터 복구
  - 30초마다 AsyncStorage에 세션 백업
  - 앱 재시작 시 복구 대화상자 표시
  - 복구 시 이전 GPS 포인트, 거리, 시간 등 모든 데이터 복원
- [x] 성능 모니터링 기능 - 주행 중 GPS 정확도, 포인트 수, 백그라운드 상태 표시
- [x] 관리자 대시보드 하얀화면 문제 수정
  - 모든 탭에 retry 및 retryDelay 옵션 추가
  - 에러 발생 시 에러 화면 표시 및 다시 시도 버튼 추가
- [ ] APK 빌드 (테스트용)

## 버그 수정 (2026-02-04)
- [x] 주행기록 중복 저장 버그 수정 - 동일한 주행 기록이 여러 번 저장되는 문제
  - saveRidingRecord에 중복 체크 로직 추가 (동일 ID 또는 유사한 데이터 1분 이내)
  - saveRideRecord에 isSaving 상태로 중복 호출 방지
- [x] 주행화면 뒤로가기 종료 시 메인화면으로 이동하도록 수정 (현재 기체선택 화면으로 이동)
  - router.back() 대신 router.replace("/(tabs)") 사용


## AI 분석 기반 개선 (2026-02-04)

### 즉시 구현 항목
- [x] Rate Limiting 미들웨어 (API 남용 방지)
- [x] Error Boundary 도입 (앱 안정성 향상)
- [x] AI 호출 조건 최적화 (짧은 주행 제외, 비용 절감)
- [x] Sentry 모니터링 연동 (실시간 오류 추적) - 에러 리포팅 유틸리티 구현
- [x] GPS 업데이트 빈도 동적 조절 (배터리 절약)

### 단기 구현 항목
- [ ] tRPC 라우터 도메인별 분할 (유지보수성 향상) - 추후 진행
- [x] RDP 알고리즘 GPS 압축 (저장 비용 80% 절감)
- [ ] useReducer 상태 관리 전환 (코드 복잡도 감소) - 추후 진행
- [x] AI 컨텍스트 압축 개선 (토큰 비용 50% 절감)
- [x] 에코 리더보드 기능 (친환경 주행 점수 및 랭킹)


## UI 개선 및 기능 연동 (2026-02-04)
- [x] 메인화면 카드 간격 일정하게 수정
- [x] 에코 리더보드 서버 API 연동
- [x] GPS 압축 적용 (주행 종료 시 compressGpsData 호출)


## 버그 수정 (2026-02-04 추가)
- [x] 관리자 대시보드 진입 시 화면이 하얗게 변하면서 벌돌 되는 현상 수정 - Error Boundary 추가 및 데이터 안전 접근 처리

## 장기 개선 항목 - AI 분석 기반 (2026-02-04)

### 클라이언트 아키텍처 개선 (성능 이슈 발생 시 검토)
- [ ] MMKV 마이그레이션 (AsyncStorage 대체) - 저장소 성능 이슈 발생 시
- [ ] Reanimated Worklets 활용 강화 - 애니메이션 성능 이슈 발생 시
- [ ] Expo Development Build(CNG) 전환 - 네이티브 모듈 필요 시

### 백엔드 및 데이터베이스 (데이터 규모 증가 시 검토)
- [ ] PostgreSQL Range Partitioning - 데이터 100만 건 이상 시
- [ ] TimescaleDB 도입 - 데이터 1000만 건 이상 시
- [ ] PostGIS 공간 인덱싱 - 위치 기반 검색 기능 추가 시

### 코드 품질 개선
- [ ] tRPC 라우터 도메인별 분할 (유지보수성 향상)
- [ ] useReducer 상태 관리 전환 (riding.tsx 코드 복잡도 감소)
- [ ] CI/CD 파이프라인 구축 (GitHub Actions 자동 빌드/배포)

### 사용자 경험 개선
- [ ] 다크 모드 완전 지원 (현재 토글만 구현)
- [ ] UI 접근성 개선 (스크린 리더 레이블, 터치 영역 확대)
- [ ] 사용자 온보딩 튜토리얼 (첫 실행 가이드)
- [ ] 푸시 알림 확장 (주간 리포트, 안전 알림)

### AI 기능 고도화 (사용자 10만 명 이상 시 검토)
- [ ] 온디바이스 ML 모델 도입 - API 비용 급증 시
- [ ] AI 분석 배치 처리 - 야간 일괄 분석으로 비용 절감

### 네트워크 최적화
- [ ] WebSocket Delta Compression - 그룹 라이딩 대역폭 절감
- [ ] Heartbeat 메커니즘 강화 - 좀비 연결 감지
- [ ] 메시지 큐 도입 - 연결 끊김 시 데이터 보존

### 보안 강화
- [ ] Redis 기반 분산 Rate Limiting - 서버 스케일 아웃 시
- [ ] API 키 로테이션 시스템
- [ ] 프라이버시 존 기능 - 민감 장소 GPS 숨김


## 기능 개선 (2026-02-04)
- [x] 관리자 대시보드 실제 테스트 확인 - TypeScript 오류 없음, 서버 정상 동작
- [x] 에코 리더보드 메인화면 진입점 추가 (Quick Access 카드)
- [x] AI 기능 월 30회 제한 시스템 구현 (챗봇, 주행통계 등 모든 AI 기능 통합)


## 기능 개선 (2026-02-04 추가)
- [x] 에코 리더보드 순위 10위까지만 공개 (나의 에코점수는 별도 유지)
- [x] 관리자 대시보드 AI 사용량 통계 추가 (전체 사용자 AI 호출 현황 및 비용 모니터링)


## 기능 개선 및 빌드 (2026-02-04)
- [x] AI 도우미 화면에 남은 횟수 UI 표시 ("월간 AI: 25/30회 남음")
- [x] APK 빌드 (실기기 테스트용) - 빌드 ID: be7770f5-76b5-474d-b87c-68abcd254781


## 버그 수정 및 기능 추가 (2026-02-04)
- [x] 관리자 대시보드 "undefined is not a function" 오류 수정 - 함수 안전 호출 처리 추가
- [x] 웹 관리자 페이지에 AI 사용량 통계 기능 추가


## 버그 수정 및 기능 추가 (2026-02-04)
- [x] 관리자 대시보드 "undefined is not a function" 오류 수정 - 함수 안전 호출 처리 추가
- [x] 웹 관리자 페이지에 AI 사용량 통계 기능 추가
- [ ] 웹 관리자 페이지에 모든 사용자 주행기록 확인 기능 추가


## 버그 수정 (2026-02-04)
- [x] 웹 관리자 페이지 사용자 ID 입력란 숫자 하나씩만 입력되는 버그 수정 - type="text"로 변경
- [x] 웹 관리자 페이지에 전체 사용자 주행기록 테이블 기능 - 이미 구현되어 있음 (주행 기록 탭)


## 버그 리포트 처리 (2026-02-04)
- [ ] 화면 꺼짐 시 주행 기록 중단 버그 수정 - 백그라운드 위치 추적 기능 구현 (신고자: 전주)
- [x] 백그라운드 위치 추적 기능 확인 - 이미 구현되어 있음 (expo-location 백그라운드 모드, 포그라운드 서비스)
- [x] 배터리 최적화 가이드 팝업 확인 - 이미 구현되어 있음 (삼성 기기 설정 안내 포함)
...
- [x] 버그 리포트 신고자(전주)에게 답변 - 배터리 최적화 설정 방법 안내 (관리자 대시보드에 답변 기능 추가)

## 버그 수정 (2026-02-04 추가)
- [x] 관리자 웹 페이지 버그 리포트 답변 기능 추가 - 답변 입력란, 전송 버튼, 기존 답변 표시 기능 추가 완료
- [x] 관리자 웹 페이지 주행 조회 데이터 반영 문제 수정 - 정상 작동 확인됨 (사용자 ID 3961751로 조회 시 주행 기록 표시됨)
- [x] 앱 내 관리자 대시보드 접속 오류 수정
  - 오류 메시지: "undefined is not a function" - toFixed() 호출 시 undefined 값 방지 처리 추가
...
## 버그 수정 (2026-02-04 추가)
- [ ] 친구 데이터가 앱에서 열람되지 않는 문제 수정
- [ ] 기체 데이터가 앱에서 열람되지 않는 문제 수정
- [ ] 커뮤니티 데이터가 앱에서 열람되지 않는 문제 수정

## 버그 수정 (2026-02-04 추가) - 서버 데이터 로딩 문제
- [ ] 날씨 정보 로딩 안됨 - "날씨 정보 로딩 중..." 상태로 멈춤
- [ ] 기체 목록 로딩 안됨 - "등록된 기체가 없습니다" 표시 (DB에는 데이터 있음)
- [ ] 커뮤니티 게시글 로딩 안됨 - "아직 게시글이 없습니다" 표시 (DB에는 데이터 있음)
- [ ] 친구 목록 로딩 안됨 - 빈 화면 표시 (DB에는 데이터 있음)
...
- [x] 그룹 라이딩 방 참가하기 오류 수정 - groupSessions, groupMembers, groupMessages 테이블 생성
- [x] 그룹 라이딩 방 나가기 오류 수정 - 테이블 생성 완료

## 버그 수정 (2026-02-04) - 관리자 대시보드 데이터 로딩 문제
...
## 다국어(한글/영어) 전환 기능 (2026-02-04)
- [x] i18n 라이브러리 설정 및 언어 파일 구조 생성
- [x] 한글/영어 번역 파일 작성
- [x] 앱 전체 텍스트를 i18n으로 전환 (탭 바, 설정 화면 주요 텍스트)
- [x] 설정 화면에 언어 선택 옵션 추가 (자동/한국어/English)
- [x] AI 도우미 영어 응답 설정 (언어 설정에 따라 영어/한국어 응답)


## 나머지 화면 i18n 전환 (2026-02-04)
- [x] 홈 화면 i18n 적용
- [x] 주행 화면 i18n 적용 (번역 키 추가 완료)
- [x] 기록 화면 i18n 적용
- [x] 커뮤니티 화면 i18n 적용
- [x] AI 도우미 화면 i18n 적용
- [x] 기타 화면 i18n 적용 (기체 관리, 그룹 라이딩, 로그인/회원가입, 랭킹, 배지) - t() 함수 적용 완료
...
## 버그 수정 (2026-02-13) - 3차
- [x] BUG: 언어 선택 메뉴에서 i18n 키가 번역되지 않고 원본 키로 표시됨 - 중첩 키 구조로 수정 (settings.language.title 등)
