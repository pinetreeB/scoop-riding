...
## 버그 수정 및 기능 추가 (2026-02-04)
- [x] 주행기록 중복 제거 기능 구현 (동일한 기록 자동 삭제)
- [x] 기존 모든 유저의 중복 주행기록 정리 (96건 → 74건)
- [x] 백그라운드 주행 종료 후 메인화면 이동 안 되는 버그 수정 (AppState 감지 추가)
- [x] 관리자 대시보드 상단 공백 레이아웃 수정 (ScrollView contentContainerStyle 수정)
- [x] 서버 이전 완료 공지 등록 (기존 장애 공지 비활성화, 정상화 공지 등록)
