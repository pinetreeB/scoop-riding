...
// Error Boundary wrapper to prevent white screen crashes
...
export default function AdminDashboardScreen() {
  return (
    <AdminErrorBoundary
      fallback={(error, reset) => <AdminErrorFallback error={error} onReset={reset} />}
    >
...
function AdminErrorFallback({ error, onReset }: { error: Error; onReset: () => void }) {
  ...
  return (
    <ScreenContainer className="items-center justify-center p-6">
      <MaterialIcons name="error-outline" size={64} color={colors.error || "#EF4444"} />
      <Text className="text-xl font-bold text-foreground mt-4">오류 발생</Text>
      <Text className="text-muted text-center mt-2 mb-6">
        관리자 대시보드를 불러오는 중 문제가 발생했습니다.{"\n"}
        {error.message}
      </Text>
      <View className="flex-row gap-3">
        <TouchableOpacity className="bg-primary px-6 py-3 rounded-xl" onPress={onReset}>
          <Text className="text-white font-semibold">다시 시도</Text>
        </TouchableOpacity>
        <TouchableOpacity className="bg-surface px-6 py-3 rounded-xl border border-border" onPress={() => router.back()}>
          <Text className="text-foreground font-semibold">돌아가기</Text>
        </TouchableOpacity>
      </View>
    </ScreenContainer>
  );
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View className="flex-1 bg-black/50 justify-end">
        <View className="bg-background rounded-t-3xl max-h-[90%]" style={{ paddingBottom: Math.max(16, insets.bottom + 8) }}>
  ...
  return (
    <View className="flex-1 p-4">
      ...
      <Modal visible={!!selectedUser} animationType="slide" transparent>
        <View className="flex-1 bg-black/50 justify-end">
          <View className="bg-background rounded-t-3xl max-h-[80%]" style={{ paddingBottom: Math.max(16, insets.bottom + 8) }}>
            <View className="flex-row items-center justify-between p-4 border-b border-border">
